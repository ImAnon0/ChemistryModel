...
class Viewer(QtWidgets.QWidget):
    ...
    def build_panel(self):
        ...
        self.slice_button = self.button(
            "Cut: off",
            self.cycle_slice
        )
        panel.addWidget(self.slice_button)

        self.axis_button = self.button("Cut axis: X", self.cycle_axis)
        ...
        self.growth_button = self.button(
            "Growth: off",
            self.toggle_growth
        )
        panel.addWidget(self.growth_button)
    ...
    def visible_mask(self, positions):
        # Which beads survive the cutting plane.
        ...
        half_width = self.box_size * 0.06

        return np.abs(coordinate - cut) <= half_width

    def toggle_growth(self):
        self.growing = not self.growing

        self.simulation.growth_interval = 150 if self.growing else 0

        self.growth_button.setText(
            "Growth: on" if self.growing else "Growth: off"
        )
